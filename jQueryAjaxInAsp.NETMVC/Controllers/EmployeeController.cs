        ...
        public ActionResult AddOrEdit(int id = 0)
        {
            ...
            if (id != 0)
            {
                using (Entities db = new Entities())
                {
                    ...
                    obj.TotalExperience = emp.TotalExperience.ToString();
            ...
            try
            {
                ...
                using (Entities db = new Entities())
                {
                    if (emp.EmployeeID == 0)
                    {
                        ...
                        obj.State = emp.State;
                        obj.Pincode = emp.Pincode;
                        obj.TotalExperience =Convert.ToInt32(emp.TotalExperience);
                    ...
                    else
                    {
                        ...
                        getEmployeeDetails.TotalExperience =Convert.ToInt32(emp.TotalExperience);
        ...
        public ActionResult Dashboard(int id)
        {
            ...
            if (id != 0)
            {
                using (Entities db = new Entities())
                {
                    ...
                    obj.Contact = Convert.ToString(emp.ContactNumber);
                    obj.TotalExperience=emp.TotalExperience.ToString();
                    if (emp.Emp_Attendance!=null)
                    {
                        var TodayAttendance = emp.Emp_Attendance.OrderByDescending(x => x.AttendanceInTime).OrderByDescending(x => x.AttendanceOutTime).FirstOrDefault();
                        obj.TotalWorkedHour = TodayAttendance!=null ? (TodayAttendance.AttendanceOutTime - TodayAttendance.AttendanceInTime).ToString() : "0";
                    }
                    obj.TotalEmployees = db.Employees.ToList().Count.ToString();
                }
            }
